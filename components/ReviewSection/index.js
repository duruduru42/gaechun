...
function Stars() {
  return <div className="text-orange-400 text-sm tracking-wide" aria-label="별점 5점">★★★★★</div>;
}

function ReviewCard({ review }) {
  return (
    <div className="bg-white rounded-[28px] p-7 md:p-8 h-full flex flex-col text-left">
      <div className="flex items-center gap-4 mb-5">
        <div className="w-16 h-16 rounded-full overflow-hidden bg-gray-50 flex items-center justify-center relative shrink-0">
          <Image src={review.logo} alt={`${review.name} 프로필`} width={44} height={44} className="object-contain" />
        </div>
        <div>
          <Stars />
          <h3 className="text-gray-900 font-extrabold text-base md:text-lg mt-1 leading-tight">{review.name}</h3>
          <span className="inline-block mt-1.5 text-[11px] font-bold text-orange-600 bg-orange-50 rounded-full px-2.5 py-0.5">
            {review.period} 컨설팅
          </span>
        </div>
      </div>
      <p className="text-gray-700 leading-relaxed text-sm md:text-[15px] flex-1">
        {review.content}
      </p>
    </div>
  );
  ...
  return (
    <section className="w-full bg-black py-20 md:py-32">
        ...
        >
          {reviews.map((review, index) => (
            <SwiperSlide key={index}>
              <div className="min-h-[460px] h-full">
      ...
      <div className="block md:hidden px-4">
        ...
        <p className="text-center text-gray-500 text-xs mt-5">← 옆으로 넘겨 더 많은 후기를 확인하세요 →</p>
      </div>
    </section>
  );
}
