...
const getConvertedScore = (percentile, subject) => {
  const inquiryType = isNaturalScience(subject) ? '자연' : '인문'; // 자연탐구, 인문탐구 구분
...
export const 아주대학교 = async (userId, selection) => {
  ...
  // 자연계열 계산
  if (selection.계열 === '자연' || selection.계열 === '의학') {
    let mathBonus = 1;
    let scienceBonus = 1;
    ...
    const scienceMaxScore = isNaturalScience(science1) && isNaturalScience(science2) ? 68.85 : 68.03;

    let totalScore =
      (standard_score_korean / 150 * 200) +
      (standard_score_math / 148 * 350 * mathBonus) +
      englishScore +
      (totalScienceConvertedScore / (2 * scienceMaxScore) * 300 * scienceBonus) +
      historyPenalty;
    ...
    if (selection.계열 === '의학') {
      totalScore =
        (standard_score_korean / 150 * 200) +
        (standard_score_math / 148 * 400 * mathBonus) +
        englishScore +
        (totalScienceConvertedScore / (2 * scienceMaxScore) * 300 * scienceBonus) +
        historyPenalty;
    }
  ...
  // 인문계열 계산
    ...
    let totalScore =
      (standard_score_korean / 150 * (selection.계열 === '인문1' ? 250 : 350)) +
      (standard_score_math / 148 * (selection.계열 === '인문1' ? 400 : 250)) +
      englishScore +
      (totalScienceConvertedScore / (2 * scienceMaxScore) * (selection.계열 === '인문1' ? 200 : 250)) +
      historyPenalty;
