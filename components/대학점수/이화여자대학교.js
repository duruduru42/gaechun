...
export const 이화여자대학교 = async (userId, selection) => {
    ...
    // 자연계열 점수 계산
    let scienceBonus = 1;
    if (isNaturalScience(science1)) scienceBonus += 0.06;
    if (isNaturalScience(science2)) scienceBonus += 0.06;

    scienceScore = (
        (standard_score_korean / 150) * 0.25 +
        (standard_score_math / 148) * 0.3 +
        englishScore * 0.2 +
        (totalScienceConvertedScore / 137.7) * 0.25 * scienceBonus
    ) * 1000 + historyBonus;

    // 최종 점수 계산
    if (selection.계열 === '인문') {
        return humanitiesScore.toFixed(2);
    } else if (selection.계열 === '자연') {
        return scienceScore.toFixed(2);
    } else if (selection.계열 === '혼합') {
        return Math.max(humanitiesScore, scienceScore).toFixed(2); // 혼합 계열일 경우 더 높은 점수 선택
    }
