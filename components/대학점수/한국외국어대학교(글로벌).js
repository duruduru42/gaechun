...
const conversionTable = {
  ...
  export const 한국외국어대학교글로벌 = async (userId, selection) => {
    ...
    const { data, error } = await supabase
      .from('exam_results')
      .select('standard_score_korean, standard_score_math, percentile_science1, percentile_science2, grade_english, grade_history')
      .eq('user_id', userId)
      .single();
    ...
    if (selection.계열 === '인문') {
      // 인문계열 계산
      const koreanScore = standard_score_korean * 0.3 * 700 / 150;
      const mathScore = standard_score_math * 0.3 * 700 / 148;
  
      const scienceScore1 = getConvertedScore(percentile_science1, '인문');
      const scienceScore2 = getConvertedScore(percentile_science2, '인문');
      const averageScienceScore = (scienceScore1 + scienceScore2) / 2;
  
      const scienceScore = (averageScienceScore / 68.85) * 0.2 * 700;
  
      totalScore = koreanScore + mathScore + scienceScore + englishScore + historyScore;
  
    } else if (selection.계열 === '자연') {
      // 자연계열 계산
      const koreanScore = standard_score_korean * 0.2 * 700 / 150;
      const mathScore = standard_score_math * 0.35 * 700 / 148;
  
      const scienceScore1 = getConvertedScore(percentile_science1, '자연');
      const scienceScore2 = getConvertedScore(percentile_science2, '자연');
      const averageScienceScore = (scienceScore1 + scienceScore2) / 2;
  
      const scienceScore = (averageScienceScore / 68.85) * 0.3 * 700;
