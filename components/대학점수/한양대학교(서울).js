...
export const 한양대학교 = async (userId, selection) => {
  ...
  if (selection.계열 === '자연') {
    baseScore = (Number(standard_score_korean) / 150) * 250 +
      (Number(standard_score_math) / 148) * 400 +
      ((Number(scienceScore1) + Number(scienceScore2)) / (68.85 * 2)) * 250;
    englishScore = getEnglishScore(grade_english, '자연');
    historyPenalty = getHistoryPenalty(grade_history, '자연');
  } else if (selection.계열 === '인문') {
    baseScore = (Number(standard_score_korean) / 150) * 350 +
      (Number(standard_score_math) / 148) * 300 +
      ((Number(scienceScore1) + Number(scienceScore2)) / (68.03 * 2)) * 250;
    englishScore = getEnglishScore(grade_english, '인문');
    historyPenalty = getHistoryPenalty(grade_history, '인문');
  } else if (selection.계열 === '상경') {
    baseScore = (Number(standard_score_korean) / 150) * 350 +
      (Number(standard_score_math) / 148) * 350 +
      ((Number(scienceScore1) + Number(scienceScore2)) / (68.03 * 2)) * 200;
    englishScore = getEnglishScore(grade_english, '인문');
    historyPenalty = getHistoryPenalty(grade_history, '인문');
  } else {
    return '불가';
  ...
  const finalScore = baseScore + englishScore - historyPenalty;
