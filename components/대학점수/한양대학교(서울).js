...
export const 한양대학교 = async (userId, selection) => {
  ...
  if (selection.계열 === '자연') {
    baseScore = (Number(standard_score_korean) / 138) * 250 +
      (Number(standard_score_math) / 144) * 400 +
      ((Number(scienceScore1) + Number(scienceScore2))) * 125 + englishScore - historyPenalty

  } else if (selection.계열 === '인문') {
    baseScore = (Number(standard_score_korean) / 138) * 350 +
      (Number(standard_score_math) / 144) * 300 +
      ((Number(scienceScore1) + Number(scienceScore2))) * 125 + englishScore - historyPenalty

  } else if (selection.계열 === '상경') {
    baseScore = (Number(standard_score_korean) / 138) * 350 +
      (Number(standard_score_math) / 144) * 350 +
