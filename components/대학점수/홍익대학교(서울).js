...
export const 홍익대학교서울 = async (userId, selection) => {
  ...
  const track = selection.계열 === '자연' ? '자연' : '인문';
  const convertedScienceScore1 = getConvertedScore(percentile_science1, track);
  const convertedScienceScore2 = getConvertedScore(percentile_science2, track);
  ...
  if (selection.계열 === '인문') {
    totalScore =
      ...
      (convertedScienceScore1 + convertedScienceScore2) * 0.25 +
      englishScore * 0.15 +
      historyScore;
  } 
  // 자연계열 계산
    ...
    totalScore =
      ...
      (convertedScienceScore1 + convertedScienceScore2) * 0.3 +
      englishScore * 0.15 +
      historyScore;
  } 
  else {
